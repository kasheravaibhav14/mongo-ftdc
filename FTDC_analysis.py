...
class FTDC_an:
    ...
    def checkMetric(self,curr_mean,prev_mean,met):
        if prev_mean[met] != 0 and (abs(curr_mean[met]-prev_mean[met])/prev_mean[met]) > 0.25:
            return True
        if met == "ss WT cache.bytes dirty in the cache cumulative" or met.startswith("ss WT concurrentTransactions"):
            return True
    ...
    def analytics(self, metricObj, queryTimestamp):
        ...
        tdelta = timedelta(seconds=30)  # 2.5 minutes delta
        for idx in range(pos, -1, -1):
            if df.iloc[idx]['serverStatus.wiredTiger.concurrentTransactions.write.available'] > self.ticketlim \
                    and df.iloc[idx]['serverStatus.wiredTiger.concurrentTransactions.read.available'] > self.ticketlim:
                print(
                    df.iloc[idx]['serverStatus.wiredTiger.concurrentTransactions.write.available'])
                tbounds['t0'] = idx
                break
        ...
        # print(df[to_monitor])
        # print(df.iloc[tbounds['p_lb']:tbounds['c_lb']+1][to_monitor])
        # print('------')
        # print(df.iloc[tbounds['c_lb']:tbounds['c_ub']+1][to_monitor])
        self.__plot(df[tbounds['p_lb']:tbounds['c_ub']+1],
                    to_monitor, vert_x=queryTimestamp)
        # self.__generatePDF(df[tbounds['p_lb']:tbounds['c_ub']+1],to_monitor,vert_x=queryTimestamp)

    def parseAll(self, queryTimeStamp=''):
        def delta(metrList):
        ...
        for met in metObj:
            if met in sel_metr_c or checkCriteria(met): #checkCriteria implements string matching for certain cumulative metrics
                data[met] = delta(metObj[met])

        for key in iter_keys:
            metObj = self.metricObj[key]
            sel_metr_c_new = [s for s in metObj.keys() if (s in sel_metr_c or checkCriteria(s))]
            ...
            '''
            new_c = [item for item in sel_metr_c_new if item not in data] # metric not in data ever before
            new_p = [item for item in sel_metr_p_new if item not in data]

            for met in new_c:
                data[met] = [0 for i in range(deltactr)] # add zeros for those metric who have never been there in the data before
            ...
            for met in sel_metr_p_new:
                data[met].extend(metObj[met]) # now fill all the values obtained
